import { Client, neonConfig } from '@jawj/test-serverless';  
// ** DO NOT RELEASE LIKE THIS **
// TODO: switch to @neondatabase/serverless when updated

import { NextRequest as Request, NextResponse as Response } from "next/server";

export const config = {
  runtime: "edge",
};

const start = Date.now();

export default async function api(req: Request) {
  const count = toNumber(new URL(req.url).searchParams.get("count"));
  const time = Date.now();

  neonConfig.wsProxy = 'ws.manipulexity.com/v1';  // TODO: delete this
  const client = new Client(process.env.NEON_DATABASE_URL);
  await client.connect();

  let data = null;
  for (let i = 0; i < count; i++) {
    data = await client.query(`SELECT "emp_no", "first_name", "last_name" FROM "employees" LIMIT 10`);
  }

  client.end();

  return Response.json(
    {
      data,
      queryDuration: Date.now() - time,
      invocationIsCold: start === time,
      invocationRegion:
        (req.headers.get("x-vercel-id") ?? "").split(":")[1] || null,
    },
    {
      headers: {
        "x-edge-is-cold": start === time ? "1" : "0",
      },
    }
  );
}

// convert a query parameter to a number
// also apply a min and a max
function toNumber(queryParam: string | null, min = 1, max = 5) {
  const num = Number(queryParam);
  return Number.isNaN(num) ? null : Math.min(Math.max(num, min), max);
}
